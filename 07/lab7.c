...
int main(int argc, char** argv) {
    ...
    for(i = 1; i <= 300; i++) {
        ...
        /* Sets the delay to wake up in one second. */
...
void get_current_time(int num) {
    ...
    clock_procfs = open("/proc/myclock2", O_RDONLY);
    clock_sysfs = open("/sys/kernel/myclock2/myclock2", O_RDONLY);

    gettimeofday(&time, NULL);

    if(clock_procfs < 0) {
        perror("Bad read file open of /proc/myclock2");
        exit(-1);
    }
...
int set_timeout_delay(int delay) {
    ...
    delay_open = open("/sys/kernel/myclock2/delay", O_RDWR);
